...
@section('main-content')
    <div class="products-managment">
        <form id="searchForm" method="GET" action="{{ route('managment-product.index') }}">
            ...
            <div style="display: flex">
                <div>
                    Рейтинг:
                    <select name="rating" onchange="submitForm()">
        ...
        @if(\Illuminate\Support\Facades\Auth::guard('admin')->user()->role_id == 1)
            <button class="profile-button" style="margin-left: 0px" onclick="createNewProduct()">Добавить</button>
        @endif
        <hr style="margin-top: 25px; margin-bottom: 25px">
                ...
                @foreach($products as $product)
                    <div class="product-item">
                        <div class="product-image">
                            <a href="{{ route('managment-product.view','id='. $product->id) }}">
                                <img src="{{ asset('images/' . $product->main_image) }}" alt="{{ $product->name }}">
                            </a>
                        </div>
                        <div>
                            <a href="{{ route('managment-product.view', 'id='.$product->id) }}">{{ $product->name }}</a>
                            ...
                            @else
                                <p>{{ $product->price }}р.</p>
                            @endif
                            <form id="managment-product-view-form" method="get" action="{{ route('managment-product.view') }}">
                                <input type="hidden" name="id" value="{{ $product->id }}">
                                <button class="submit-button" type="submit">Редактировать</button>
                            </form>
                        </div>
                    </div>
                @endforeach
